...
class EventsSubscriber implements EventSubscriberInterface {
  ...
  public static function getSubscribedEvents() {
    ...
    $events[IncidentEvents::NEW_REPORT][] = ['alphaTest'];
    ...
    $events[IncidentEvents::NEW_REPORT][] = ['betaTest', -100];
   ...
   */
  public function alphaTest(IncidentReportEvent $event) {
    ...
    if ($event->getType() == 'stolen_princess') {
      //\Drupal::logger('module')->notice("TEST2");
      drupal_set_message($this->t('Mario diop has been alerted. Thank you. This message was set by an event subscriber. See \Drupal\module\EventSubscriber\EventsSubscriber::notifyMario()'), 'status');
    }
    if ($event->getType() == 'cat') {
      drupal_set_message($this->t('Mous bou mak!! has been alerted. Thank you. This message was set by an event subscriber. See \Drupal\module\EventSubscriber\EventsSubscriber::notifyMario()'), 'status');
    }
  }
   ...
   *   The event object containing the incident report.
   */
  public function betaTest(IncidentReportEvent $event) {
    if ($event->getType() == 'cat') {
      drupal_set_message($this->t('Mouss bou ndaw has been alerted. Thank you. This message was set by an event subscriber. See \Drupal\module\EventSubscriber\EventsSubscriber::notifyBatman()'), 'status');
